...
lm.pca.crap <- prcomp(mod.resid)
summary(lm.pca.crap)
dgrp.loading.for.cor <- lm.pca.crap[["rotation"]][,1:3]

colnames(dgrp.loading.for.cor) <- c("dgrp.PC1", "dgrp.PC2", "dgrp.PC3")

write.csv(dgrp.loading.for.cor, file = "../Data/dgrp_all_first3PC.csv", quote = FALSE)
