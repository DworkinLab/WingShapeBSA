...
WingPlot <- function( PrcCrds, wingcol = "blue", lines = TRUE, add = FALSE, wingcex = 0.75, winglwd = 1, winglty = 1, wingpch = 1, LM_highlit = FALSE, LM_pch = 1 ){
	...
	plot( LMx.mean, LMy.mean, 
	      ...
	      asp = 1, frame.plot = F, ann = F, axes = F,
	      cex = 0.3, type = "n", xaxt = "n", yaxt = "n")
	...
	if(lines == TRUE){
    	...
    	lines( c(LMx.mean[from[i]], LMx.mean[to[i]]), 
    	       ...
    	       col = wingcol, lty = winglty, lwd = winglwd)
		}}

	if( LM_highlit == TRUE ) {
	  lmco <- c(1:13, 30)
	  points( LMx.mean[ lmco ], LMy.mean[ lmco ], 
	          cex = wingcex, pch = LM_pch)
	}}
...
WingBlur <- function ( PrcCrds, wingcol = "#add8e632", winglwd = 3, winglty = 1, wingpch = 1, wingcex = 0.9, meanline = TRUE, meancol = "blue", meanlty = 1, meanlwd = 2 ) {
	...
	plot( LMx.mean, LMy.mean, 
	      ...
	      frame.plot = F, axes = F, ann = FALSE, asp = 1)

	# nested loops to draw each line segment for each row in shape.X matrices
	for (j in 1:nrow(shape.X)){
		for (i in 1:length(from)) {
		lines( c(shape.X[j,from[i]], shape.X[j,to[i]]), 
		       c(shape.Y[j,from[i]], shape.Y[j,to[i]]), 
		       col = wingcol, lty = winglty, lwd = 3)
		}}
...
################## WingBlur2, refactoring to use rgb directly instead of hexmode

WingBlur2 <- function ( PrcCrds, wingcol = rgb(0, 0, 1, 0.05), winglwd = 3, winglty = 1, wingpch = 1, wingcex = 0.9, meanline = TRUE, meancol = rgb(0, 0, 0.8, 1), meanlty = 1, meanlwd = 2 ) {
	# set up required coord vectors
	from <- c( 6, 47, 5, 29, 5, 28, 27, 26, 25, 4, 24, 23, 3, 22, 2, 21, 20, 19, 1, 18, 17, 16, 15, 14, 12, 33, 32, 7, 31, 8, 48, 37, 9, 36, 8, 35, 34, 30, 11, 42, 10, 41, 40, 39, 38, 10, 11, 46, 45, 44, 43 )
	...
	plot( LMx.mean, LMy.mean, 
	      xlim = c(-0.25, 0.25), ylim = c(-0.25, 0.25), 
	      type = "n", xaxt = "n", yaxt = "n", 
	      asp = 1, ann = F, frame.plot = F, axes = F)
...
comment(WingBlur2) <- c("This draws a wing outline&veins for each row of the matrix 'PrcCrds' - can be individuals, line means, whatever... the default setting are for translucent lines so that the coloured area represents that phenotypic space sampled by the wings measured. Mean shape overlaid as points + outline, but can be suppressed")

##################



################## WingBlur3, allowing two colours for two groups

# For plotting the colours of two groups, set groups TRUE, and the grouping_var should be the vector (for however many wings) of the dummy variable
# This is still fragile AF, but will work for the limited case for the moment. Not for general use yet.
# add in smoothScatter functionallity for points only?

WingBlur3 <- function ( PrcCrds, groups = FALSE, grouping_var, winglwd = 3, winglty = 1, wingpch = 1, wingcex = 0.9, meanline = TRUE, meanlty = 1, meanlwd = 2, blur_transparency = 0.05 ) {
	
	if (groups == F) {wingcol = rgb(0, 0, 1, blur_transparency)
		              meancol = rgb(0, 0, 0.8, 1)
		              grouping_var = 1}  
	else {wingcol = c(rgb(0, 0, 1, blur_transparency), rgb(1, 0, 0, blur_transparency))
	  	    meancol = rgb(0, 0, 0, 0.4)
	...
	# nested loops to draw each line segment for each row in shape.X matrices
	# adds grouping variable in colour
	for (j in 1:nrow(shape.X)){
		for (i in 1:length(from)) {
		lines( c(shape.X[j,from[i]], shape.X[j,to[i]]), 
		       c(shape.Y[j,from[i]], shape.Y[j,to[i]]), 
		       col = wingcol[grouping_var[j]], lty = winglty, lwd = 3 )
		}}
...
comment(WingBlur3) <- c("Please see comments in the code. This is still a very fragile and limited function!")
...
WingEffect <- function ( meanshape, effectplus, effectminus, wingcol = c("black", "black", "red"), winglwd = c(2, 2, 2), winglty = c(1, 1, 1), scale.factor = 2, meanline = FALSE, wingcex = c(0,0,0), winglabel = "", add = FALSE, wingframe = T, scale.display = T, wingpoints = F, wingpch = c(16,1,1) ) {
	...
	# plot mean shape
		...
		plot( LMx.mean, LMy.mean, 
		      xlim = c(-0.22, 0.27), ylim = c(-0.3, 0.27), 
		      type = "n", xaxt = "n", yaxt = "n", 
		      asp = 1,
		      ann = FALSE, frame = wingframe)
		}
		if (meanline == TRUE){
	    ...
	    if (wingpoints == TRUE) {
	    	points( x.plus, y.plus, cex = wingcex[2], 
	    	        pch = wingpch[2], col = wingcol[2])
	      
	    	points( x.minus, y.minus, cex = wingcex[3], 
	    	        pch = wingpch[3], col = wingcol[3])
	    }
	...
	for (i in 1:length(from)) {
		lines( c(x.plus[from[i]], x.plus[to[i]]), 
		       c(y.plus[from[i]], y.plus[to[i]]), 
		       col = wingcol[2], lty = winglty[2], lwd = winglwd[2] )
	  
		lines( c(x.minus[from[i]], x.minus[to[i]]), 
		       c(y.minus[from[i]], y.minus[to[i]]), 
		       col = wingcol[3], lty = winglty[3], lwd = winglwd[3] )
	}
		
	# adding an annotation of the effect scaling factor
	if (scale.display == T) {
		text( 0.025, -0.215, 
		      paste("scale factor =", as.character(scale.factor)) )
		}
...
WingMovie <- function ( meanshape, effect, wingcol = c("black", "red"), winglwd = c(2, 2), winglty = c(1, 1), wingcex = 1, scale.factor = 2, effectlines = FALSE, meanline = TRUE, winglabel="" ) {
	...
	for (j in 1:length(multiplier)) {
		...
		plot( LMx.mean, LMy.mean, 
		      xlim = c(-0.22, 0.27), ylim = c(-0.22, 0.27), 
		      cex = wingcex, col = wingcol[1], xaxt = "n", yaxt = "n", 
		      ann = FALSE, asp = 1) 	# sets up plot, adds mean points
		...
		for (i in 1:length(from)) {
			if (meanline==TRUE) {
				lines( c(LMx.mean[from[i]], LMx.mean[to[i]]), 
				       c(LMy.mean[from[i]], LMy.mean[to[i]]), 
				       col = wingcol[1], lty = winglty[1], lwd = winglwd[1] )
				}
		    	lines( c(plotX[from[i]], plotX[to[i]]), 
		    	       c(plotY[from[i]], plotY[to[i]]), 
		    	       col = wingcol[2], lty = winglty[2], lwd = winglwd[2])# draws the outline segments for each 'from-to' pair
		    	
    		if (effectlines == TRUE){
   				for (j in 1:length(LMx.mean)) {
					lines( c(x.plus[j], x.minus[j]), 
					       c(y.plus[j], y.minus[j]), 
					       col = "black", lwd = 1)
...
for (j in 1:length(multiplier)) {	 #i.e. for each frame of the animation
	...
	plot( LMx.mean, LMy.mean, 
	      xlim = c(-0.22,0.27), ylim = c(-0.22,0.27), 
	      cex = 1, col = "black", xaxt = "n", yaxt = "n", 
	      ann = FALSE, asp = 1) 	#sets up the plot and adds mean points
	#text( 0, 0.25, "brk")		#labels the plot with 'SNP'
    	...
    	lines( c(plotX[from[i]], plotX[to[i]]), 
    	       c(plotY[from[i]], plotY[to[i]]), 
    	       col = "red", lty = 1, lwd = 3 )	#draws the outline segments for each 'from-to' pair
