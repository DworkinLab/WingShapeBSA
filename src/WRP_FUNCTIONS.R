...
# This is just the magnitude/L2 of the vector!

PD <- function( x ) {
				sqrt( t( x ) %*% x )}
comment(PD) <- c("This just computes the Euclidean Distance (L2 norm) for a vector")


#########Vector Correlations #######

# For Vector correlations near -1 or 1, the Fisher Z transformed value of the Pearson correlation coefficient may be advisable, in particular for generating confidence intervals.i.e. resample z, compute confidence limits, and transform these back to r.
...
# This function will compute the angle between two vectors.	BEWARE if the vectors can be of arbitrary sign, use the function below this one. This is equivalent to the Pearson correlation coefficient.

ang.vec <- function(vec1, vec2){
	vec1 <- vec1 - mean(vec1) # Centering vector to enable comparison to Pearson Corr
	vec2 <- vec2 - mean(vec2) # Centering vector to enable comparison to Pearson Corr
	vec.cor <- (t(vec1) %*% vec2)/(PD(vec1)*PD(vec2))
	vec.angle <- acos(vec.cor)*(180/pi)
	return(c(vector.cor=vec.cor, vec.angle=vec.angle))}
comment(ang.vec) <- c(" This computes both the vector correlation, and angle, between two vectors.", " to compare to the Pearson correlation coefficient make sure to center vectors", "DO NOT USE THIS IF VECTORS CAN BE OF ARBITRARY SIGN")
...
ang.vec.abs <- function(vec1, vec2){
	...
	vec.angle <- acos(vec.cor)*(180/pi)
	return(c(vector.cor=vec.cor, vec.angle=vec.angle))}
comment(ang.vec.abs) <- c(" This computes both the vector correlation, and angle, between two vectors.", " to compare to the Pearson correlation coefficient make sure to center", "set it up to compute the absolute values of the vector correlation")

##### Faster approach to compute vector correlation. 
# note that using the cor function is slow.
# This function below is written for speed, and is likely fragile, and unclear to others!
# use only for iterative boostrap for studentized bootstrap CIs.
# it only computers vector correlations.
...
# Somehow we ended up with two shape score functions. Check them and merge if necessary.

# Klingenberg's "shape score". i.e. a projection of the observed data in the direction of the vector of model coefficients
...
comment(ShapeScore) <- c("This computes the 'shape score' of Drake and Klingenberg 2008", "This is simply the projection of the raw data Y onto a vector Beta",
  "Beta is the coefficients for each procrustes coordinate from a multivariate multiple regression. ",
  "i.e. shape.model.1 <- lm(cbind(Y1, Y2) ~ X1 + X2, data=data.set )",
  "Beta ...   beta.model <- t(coef(shape.model.1)[k,]) # where k is the covariate of focus",
  "Y is the raw procrustes coordinates (Y1, Y2, ...)",
...
# There is a `proj` function that may work.
# revise this to use solve, to speed it up (for bootstrap)
